...
def format_tools_help_text() -> str:
    return "\n".join(
        [
            ...
            "Configure: tools.allowWrite, tools.allowShell, tools.allowNetwork,",
            "  readRoots, networkAllowlist in config.yaml",
            "",
            "Assistant proposes tools via ```tool JSON blocks in replies.",
            "Read-only tools may auto-run; write/shell/network/action tools need approval.",
            "",
            "fetch_url does an HTTP GET of an allowlisted domain (tools.networkAllowlist).",
            "It is off unless tools.allowNetwork is true, blocks private/loopback IPs,",
...
def format_agents_help_text() -> str:
    return "\n".join(
        [
            ...
            "",
            "  /agents resume [run_id]",
            "    Continue a paused or blocked run after approving/rejecting its",
            "    checkpoints or editing a task. Resolved tool results are fed back",
            ...
            "  attempt. After a failed repair, strictJson=true blocks the task;",
            "  strictJson=false accepts the raw text as the task result instead.",
            "",
            "Context pruning:",
            ...
            "  through context_pruning.exclude_sibling_tasks. Local context can also be",
            "  injected per task: include_rag pulls indexed documents (on by default for",
            "  the researcher role), include_memory adds user/project/session memory, and",
            "  include_skills lists active skills. All are best-effort and never block a run.",
            "",
            "Tool safety:",
            ...
            "  writeRoots, shellAllowlist, and the existing tool audit log. Each role is",
            "  further scoped by agents.roles.<role>.allowedTools (empty = no restriction);",
            "  a disallowed tool request is refused before it can run or pause the run.",
            "",
            "Configuration:",
            "  agents.runsPath        Where run JSON files are stored.",
            "  agents.maxIterations   Revision limit before blocking.",
            "  agents.strictJson      Block on schema failure (true) or accept raw text (false).",
            "  agents.residencyMode   hybrid or sequential model loading.",
            "  agents.modelProfiles   orchestrator, creator, critic_executor, etc.",
            "  agents.roles           Maps each role to a model profile and allowedTools.",
            ...
            "  6. /agents resume to continue after edits or checkpoint decisions",
            "",
            "Back to all commands: /help",
        ]
    )
